...
import { codesIntersection, codesUnion } from "./codes";
...
import {
  ...
  NO_CONSENT,
  flipConsentDecision
} from "./consent-decisions";
import { processProvision, matchPurposeOfUse } from "./consent-provisions";
...
const period = (consent) =>
  consent?.period || consent?.provision?.period; //R5 //R4;
...
function defaultDecision(consent) {
  const baseDecision =
    consent?.decision || consent?.provision?.type; //R5 //R4
