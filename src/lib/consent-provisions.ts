import { codesIntersection } from "./codes";
import { identifiersIntersection } from "./identifiers";
import superagent from "superagent";
import { maybeAddAuth } from "./auth";
import CONSENT_VALUESETS from "./consent-valuesets";
import { CONSENT_PERMIT, CONSENT_DENY } from "./consent-decisions";
...
function matchPurposeOfUse(provision, query) {
  ...
  const queriedPurposes = untrimmedQueriedPurposes.map((purpose) => ({
    system: CONSENT_VALUESETS.PURPOSE_OF_USE_SYSTEM,
    code: purpose
  }));
...
async function processProvision(
  ...
  const matchedActor = await matchActor(provision, query, fhirBase);
...
function determineObligations(query, decision, provision) {
  ...
  const obligation =
    decision === CONSENT_PERMIT
      ? CONSENT_VALUESETS.CONSENT_OBLIGATIONS.CODES.EXCEPT(allCodes)
      : decision === CONSENT_DENY
        ? CONSENT_VALUESETS.CONSENT_OBLIGATIONS.CODES.ONLY(allCodes)
        : null;

  return allCodes.length && obligation ? [obligation] : [];
}

export { processProvision, matchPurposeOfUse };
